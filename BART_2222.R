...
real_train_val = data$SPREAD[3:c(num_train+2)]
real_test_val = data$SPREAD[c(num_train + 2):num_total]
diff_real_train = diff(real_train_val)
diff_real_test = diff(real_test_val)
set.seed(20190225)
x_train = data.frame(
  DGS10_DIFF1 = feature1[3:c(num_train + 1)],
  VIX_DIFF1 = feature2[3:c(num_train + 1)],
  SLOPE = feature3[3:c(num_train + 1)],
  SKEW_DIFF1 = feature4[3:c(num_train + 1)],
  SP500_R = feature5[3:c(num_train + 1)],
  SWAPSPREAD_DIFF1 = feature6[3:c(num_train + 1)],
  SPREAD = data$SPREAD[3:c(num_train + 1)],
  SPREAD_1DAY_BEFORE = data$SPREAD[2:num_train],
  SPREAD_2DAY_BEFORE = data$SPREAD[1:c(num_train - 1)]
)
x_test = data.frame(
  DGS10_DIFF1 = feature1[c(num_train + 2): c(num_total - 1)],
  VIX_DIFF1 = feature2[c(num_train + 2): c(num_total - 1)],
  SLOPE = feature3[c(num_train + 2): c(num_total - 1)],
  SKEW_DIFF1 = feature4[c(num_train + 2): c(num_total - 1)],
  SP500_R = feature5[c(num_train + 2): c(num_total - 1)],
  SWAPSPREAD_DIFF1 = feature6[c(num_train + 2): c(num_total - 1)],
  SPREAD = data$SPREAD[c(num_train + 2): c(num_total - 1)],
  SPREAD_1DAY_BEFORE = data$SPREAD[c(num_train + 1): c(num_total - 2)],
  SPREAD_2DAY_BEFORE = data$SPREAD[num_train: c(num_total - 3)]
)
...
lines(x_axis_train,y_train_pred,col="red",lwd=1.5,lty=2)
...
plot(x_axis_test,real_test_val,
     cex=.3,cex.axis=.8,cex.lab=.7, mgp=c(1.3,.3,0),tcl=-.2,lty=1)
lines(x_axis_test,y_test_pred,col="red",lwd=1.5,lty=2)
...
MSE = mean((y_test_pred - real_test_val)^2)
MSE_train = mean((y_train_pred - real_train_val)^2)
pred_train = y_train_pred
pred_test = y_test_pred
diff_pred_test = apply(yfit$yhat.test,2,mean)
